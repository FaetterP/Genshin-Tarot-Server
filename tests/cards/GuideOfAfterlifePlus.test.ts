...
describe("GuideOfAfterlifePlus — получить 1 урон, +энергия или 2 карты, атаки накладывают Пиро в этот ход", () => {
  let game: TestGame;
  ...
  it("основной режим: игрок получает 1 урон, +1 энергия, добавляется эффект GuideOfAfterlife", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.GuideOfAfterlifePlus);
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.GuideOfAfterlifePlus }),
      }),
    );
  ...
  it("Attack-карта с активным эффектом накладывает Пиро на врага", async () => {
    ...
    const guideId = await ensureCardInHand(player, admin, ECard.GuideOfAfterlifePlus);
  ...
  it("второй игрок получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.GuideOfAfterlifePlus);
    ...
    expect(response1.card).toBe(ECard.GuideOfAfterlifePlus);
    expect(response2.card).toBe(ECard.GuideOfAfterlifePlus);
