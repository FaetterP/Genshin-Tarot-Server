...
describe("KatzleinStylePlus — наносит 2 пронзающих урона, или 5 если у игрока есть защита", () => {
  let game: TestGame;
  ...
  it("наносит 2 пронзающих урона когда у игрока нет защиты", async () => {
    ...
    await admin.updatePlayer(player.playerId, { shield: 0 });

    const cardId = await ensureCardInHand(player, admin, ECard.KatzleinStylePlus);
  ...
  it("наносит 5 пронзающих урона когда у игрока есть защита", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
    ...
    const enemy = player.enemies[0];
    await admin.updateEnemy(enemy.id, { hp: 10, shield: 0, elements: [] });
    await admin.updatePlayer(player.playerId, { shield: 3 });
  ...
  it("уменьшает AP на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.KatzleinStylePlus }),
      }),
    );
  });

  it("пронзает щит врага — HP уменьшается даже при наличии щита", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("isRange: может бить врага в зоне другого игрока", async () => {
    ...
    await admin.updatePlayer(player1.playerId, { shield: 0 });

    const cardId = await ensureCardInHand(player1, admin, ECard.KatzleinStylePlus);
  ...
  it("возвращает ошибку при недостаточном количестве AP", async () => {
    ...
    const enemy = player.enemies[0];
    await admin.updateEnemy(enemy.id, { hp: 10, shield: 0, elements: [] });
    await admin.updatePlayer(player.playerId, { actionPoints: { normal: 0, extra: 0 }, shield: 0 });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    expect(response1.card).toBe(ECard.KatzleinStylePlus);
    expect(response2.card).toBe(ECard.KatzleinStylePlus);
