/**
 * SolarIsotomaPlus — Накладывает Гео всем врагам в вашей зоне. В начале следующего хода накладывает Гео
...
import { SolarIsotomaPlus } from "../../src/storage/cards/Albedo/SolarIsotomaPlus";
...
describe("SolarIsotomaPlus — Накладывает Гео всем врагам в зоне, даёт эффект Тектоническая волна. Опционально: выбросить карту и вытянуть", () => {
  let cycleController: CycleController;
  let player: Player;
  let card: SolarIsotomaPlus;
  let ws: ExtWebSocket;
  ...
  it("рассылает шаги: EnemyGetElement → PlayerGetEffect → PlayerStatChange(ap, -1) → MoveCard, эффект добавлен", async () => {
  ...
  it("два врага в зоне — EnemyGetElement для каждого в порядке добавления", async () => {
