/**
 * FavoniusBladeworkPlus - Наносит 2 урона 1 врагу в вашей зоне и получите 2 энергии. Восстанавливает вам 2 ОЗ.
...
describe("FavoniusBladeworkPlus — наносит 2 урона, даёт 2 энергии, восстанавливает 2 ОЗ", () => {
  let game: TestGame;
  ...
  it("наносит 2 урона, даёт 2 энергии и восстанавливает 2 ОЗ", async () => {
    ...
    await admin.updatePlayer(player.playerId, { actionPoints: { normal: 3, extra: 0 }, energy: 0, hp: 8 });

    const cardId = await ensureCardInHand(player, admin, ECard.FavoniusBladeworkPlus);
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.FavoniusBladeworkPlus }),
      }),
    );
    ...
    expect(response.player.hp).toBe(10);
  });

  it("щит блокирует урон — HP врага не изменяется, энергия и лечение всё равно начисляются", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("убивает врага с 2 HP и спавнит следующую волну", async () => {
    ...
    const enemy = player.enemies[0];
    await admin.updateEnemy(enemy.id, { hp: 2, shield: 0 });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.FavoniusBladeworkPlus);
    ...
    expect(response1.card).toBe(ECard.FavoniusBladeworkPlus);
    expect(response2.card).toBe(ECard.FavoniusBladeworkPlus);
