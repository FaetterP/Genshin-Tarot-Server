/**
 * GaleBlade - Накладывает Анемо и наносит 1 пронзающий урон 1 врагу в вашей зоне. Можно поменять местами этого врага с любым другим.
...
describe("GaleBlade — накладывает Анемо и наносит 1 пронзающий урон, можно поменять врагов местами", () => {
  let game: TestGame;
  ...
  it("накладывает Анемо и наносит 1 пронзающий урон без свопа", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.GaleBlade);
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.GaleBlade }),
      }),
    );
  ...
  it("меняет местами врага игрока 1 с врагом игрока 2", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.GaleBlade);
  ...
  it("возвращает ошибку если первый враг в свопе не из зоны игрока", async () => {
    game = await createTestGame(2);
    const [player1, player2] = game.players;
    const { admin } = game;
    ...
    // первый враг из зоны игрока 2 — не может быть целью атаки при свопе
    ...
    expect(response.status).toBe("error");
    expect(response.message).toBe("first enemy must be in your zone");
  });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    expect(response1.card).toBe(ECard.GaleBlade);
    expect(response2.card).toBe(ECard.GaleBlade);
