/**
 * PassionOverloadPlus - Накладывает Пиро и наносит 4 урона 1 врагу в вашей зоне.
...
describe("PassionOverloadPlus - Накладывает Пиро и наносит 4 урона 1 врагу. Спецэффект при карте Беннета или Горения сверху колоды.", () => {
  let game: TestGame;
  ...
  it("наносит 4 Пиро урона и сбрасывает обычную карту сверху колоды", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.PassionOverloadPlus);
  ...
  it("уменьшает AP игрока на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        type: EDetailedStep.MoveCard,
        to: "discard",
        card: expect.objectContaining({ name: ECard.PassionOverloadPlus }),
      }),
    );
  });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.PassionOverloadPlus);
    ...
    expect(response1.card).toBe(ECard.PassionOverloadPlus);
    expect(response2.card).toBe(ECard.PassionOverloadPlus);
