...
describe("Frostgnaw — накладывает Крио на всех врагов в зоне, даёт 1 энергию", () => {
  let game: TestGame;
  ...
  it("накладывает Крио на врага и добавляет шаг EnemyGetElement", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.Frostgnaw);
  ...
  it("даёт 1 энергию игроку — шаг PlayerStatChange и energy в состоянии", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("уменьшает AP на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.Frostgnaw }),
      }),
    );
    expect(response.player.discard.some((c: any) => c.name === ECard.Frostgnaw)).toBe(true);
  });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.Frostgnaw);
    ...
    expect(response1.card).toBe(ECard.Frostgnaw);
    expect(response2.card).toBe(ECard.Frostgnaw);
