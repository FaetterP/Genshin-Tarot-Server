/**
 * ExplosivePuppetPlus - Выбранный 1 враг, не босс, в вашей зоне не будет атаковать в этом ходу.
...
describe("ExplosivePuppetPlus — оглушает врага в зоне, в начале следующего цикла наносит 2 Pyro урона всем врагам", () => {
  let game: TestGame;
  ...
  it("оглушает врага и вешает эффект игроку", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.ExplosivePuppetPlus);
  ...
  it("уменьшает AP на 1, карта перемещается в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.ExplosivePuppetPlus }),
      }),
    );
    expect(response.player.discard.some((c: any) => c.name === ECard.ExplosivePuppetPlus)).toBe(true);
  });
  ...
  it("возвращает ошибку если враг вне зоны игрока", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.ExplosivePuppetPlus);
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    expect(response1.card).toBe(ECard.ExplosivePuppetPlus);
    expect(response2.card).toBe(ECard.ExplosivePuppetPlus);
