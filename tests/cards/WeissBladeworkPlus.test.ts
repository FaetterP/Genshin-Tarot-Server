...
import { ECard, EDetailedStep, EPlayerEffect } from "../../src/types/enums";
import {
  startTestServers,
  stopTestServers,
  resetGame,
...
describe("WeissBladeworkPlus — наносит 3 урона 1 врагу в зоне, с эффектом SolarIsotoma даёт AP и карту", () => {
  let game: TestGame;
  ...
  it("наносит 3 урона врагу (без эффекта SolarIsotoma)", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.WeissBladeworkPlus);
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.WeissBladeworkPlus }),
      }),
    );
  ...
  it("с эффектом SolarIsotoma: наносит 3 урона, получает 1 AP и вытягивает карту", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
    ...
    // Применяем SolarIsotomaPlus чтобы повесить эффект (1 AP)
    const solarCardId = await ensureCardInHand(player, admin, ECard.SolarIsotomaPlus);
    player.send({ action: "game.useCard", cardId: solarCardId });
    const solarResponse = await player.waitFor((m: any) => m.action === "game.useCard");
    expect(solarResponse.player.effects).toContain(EPlayerEffect.SolarIsotoma);
  ...
  it("атака блокируется щитом — HP не изменяется, оба шага присутствуют", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.WeissBladeworkPlus);
    ...
    expect(response1.card).toBe(ECard.WeissBladeworkPlus);
    expect(response2.card).toBe(ECard.WeissBladeworkPlus);
