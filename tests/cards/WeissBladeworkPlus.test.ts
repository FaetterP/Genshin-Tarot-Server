/**
 * WeissBladeworkPlus — Наносит 3 урона 1 врагу в вашей зоне. Если у вас есть эффект Тектоническая волна,
 * на этом ходе: получите 1 Очко действия и вытяните 1 карту.
 * Тип: Attack. Стоимость: 1 очко действия.
 */

import gameHandlers from "../../src/ws/handlers/game";
import { sendToAll } from "../../src/utils/wsUtils";
import { WeissBladeworkPlus } from "../../src/storage/cards/Albedo/WeissBladeworkPlus";
...
describe("WeissBladeworkPlus — Наносит 3 урона 1 врагу в вашей зоне. Если есть эффект Тектоническая волна: +1 AP и вытянуть 1 карту", () => {
  let cycleController: CycleController;
  let player: Player;
  let card: WeissBladeworkPlus;
  let ws: ExtWebSocket;
  ...
  it("с эффектом Тектоническая волна: EnemyTakeDamage → PlayerStatChange(ap, +1) → DrawCards → PlayerStatChange(ap, -1) → MoveCard", async () => {
    player.addEffect(new SolarIsotomaEffect());
    const enemy = addEnemy(7);
    const deckCard = new WeissBladeworkPlus();
    ...
    const extraApIdx = steps.findIndex(
      (s) =>
        s.type === EDetailedStep.PlayerStatChange &&
        s.stat === "actionPoints" &&
        s.delta === 1 &&
    ...
    expect(damageIdx).toBeGreaterThanOrEqual(0);
    expect(extraApIdx).toBeGreaterThanOrEqual(0);
    expect(drawIdx).toBeGreaterThanOrEqual(0);
    expect(apIdx).toBeGreaterThanOrEqual(0);
    expect(discardIdx).toBeGreaterThanOrEqual(0);
    expect(damageIdx).toBeLessThan(extraApIdx);
    expect(extraApIdx).toBeLessThan(drawIdx);
    ...
    expect(steps[extraApIdx]).toEqual({
      type: EDetailedStep.PlayerStatChange,
      stat: "actionPoints",
      playerId: player.ID,
      delta: 1,
  ...
  it("без эффекта Тектоническая волна — шаги PlayerStatChange(ap, +1) и DrawCards отсутствуют", async () => {
    const enemy = addEnemy(7);
