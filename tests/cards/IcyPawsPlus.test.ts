...
describe("IcyPawsPlus — накладывает Крио на всех врагов в зоне, даёт 3 защиты и выбрасывает Горение из сброса", () => {
  let game: TestGame;
  ...
  it("даёт 3 защиты и накладывает Крио на врага одновременно", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.IcyPawsPlus);
  ...
  it("накладывает Крио на каждого врага при нескольких врагах в зоне", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("выбрасывает карту Горение из сброса", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("работает без ошибок если Горения нет в сбросе", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("уменьшает AP на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        type: EDetailedStep.MoveCard,
        to: "discard",
        card: expect.objectContaining({ name: ECard.IcyPawsPlus }),
      }),
    );
  });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.IcyPawsPlus);
    ...
    expect(response1.card).toBe(ECard.IcyPawsPlus);
    expect(response2.card).toBe(ECard.IcyPawsPlus);
