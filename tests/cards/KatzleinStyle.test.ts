...
describe("KatzleinStyle — наносит 2 пронзающих урона 1 врагу где угодно", () => {
  let game: TestGame;
  ...
  it("наносит 2 пронзающих урона врагу (враг выживает)", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.KatzleinStyle);
  ...
  it("уменьшает AP на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.KatzleinStyle }),
      }),
    );
  });
  ...
  it("isRange: может бить врага в зоне другого игрока", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.KatzleinStyle);
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    expect(response1.card).toBe(ECard.KatzleinStyle);
    expect(response2.card).toBe(ECard.KatzleinStyle);
