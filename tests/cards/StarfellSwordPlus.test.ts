...
describe("StarfellSwordPlus — накладывает Гео и наносит 2 пронзающего урона всем врагам в зоне", () => {
  let game: TestGame;
  ...
  it("наносит 2 пронзающего Гео урона врагу", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.StarfellSwordPlus);
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.StarfellSwordPlus }),
      }),
    );
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.StarfellSwordPlus);
    ...
    expect(response1.card).toBe(ECard.StarfellSwordPlus);
    expect(response2.card).toBe(ECard.StarfellSwordPlus);
