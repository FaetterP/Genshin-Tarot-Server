...
describe("StrikeOfFortune - Наносит 2 урона 1 врагу в вашей зоне. Даёт 2 энергии, если на враге есть Пиро.", () => {
  let game: TestGame;
  ...
  it("наносит 2 урона врагу без щита", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.StrikeOfFortune);
  ...
  it("даёт 2 энергии если на враге есть Пиро", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;

    const enemy = player.enemies[0];
    await admin.updateEnemy(enemy.id, { hp: 10, shield: 0, elements: [EElement.Pyro] });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.StrikeOfFortune);
    ...
    expect(response1.card).toBe(ECard.StrikeOfFortune);
    expect(response2.card).toBe(ECard.StrikeOfFortune);
