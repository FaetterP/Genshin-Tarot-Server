/**
 * SearingOnslaughtPlus - Накладывает Пиро и наносит 2 урона 1 врагу в вашей зоне.
...
describe("SearingOnslaughtPlus - Накладывает Пиро и наносит 2 урона 3 раза 1 врагу в зоне", () => {
  let game: TestGame;
  ...
  it("наносит 3 удара по 2 Пиро урона врагу без щита", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.SearingOnslaughtPlus);
  ...
  it("уменьшает AP игрока на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.SearingOnslaughtPlus }),
      }),
    );
  });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.SearingOnslaughtPlus);
    ...
    expect(response1.card).toBe(ECard.SearingOnslaughtPlus);
    expect(response2.card).toBe(ECard.SearingOnslaughtPlus);
