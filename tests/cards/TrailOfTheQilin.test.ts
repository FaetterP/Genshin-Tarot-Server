/**
 * TrailOfTheQilin - Выбранный враг в вашей зоне не атакует в этом ходу.
...
describe("TrailOfTheQilin — оглушает врага в зоне, в начале следующего цикла Крио + 2 урона всем врагам", () => {
  let game: TestGame;
  ...
  it("оглушает врага и вешает эффект игроку", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.TrailOfTheQilin);
  ...
  it("уменьшает AP на 2, карта перемещается в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.TrailOfTheQilin }),
      }),
    );
  });
  ...
  it("щит блокирует урон эффекта (не пронзающий)", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;

    const enemy = player.enemies[0];
    await admin.updateEnemy(enemy.id, { hp: 20, shield: 5, elements: [] });
    await admin.updatePlayer(player.playerId, { actionPoints: { normal: 3, extra: 0 } });
    ...
    expect(cycle2.you.enemies.find((e: any) => e.id === enemy.id).hp).toBe(20);
  });

  it("в начале следующего цикла бьёт всех врагов в зоне (2 врага)", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
  ...
  it("возвращает ошибку если враг вне зоны игрока", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.TrailOfTheQilin);
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    expect(response1.card).toBe(ECard.TrailOfTheQilin);
    expect(response2.card).toBe(ECard.TrailOfTheQilin);
