/**
 * StrikeOfFortunePlus - Наносит 3 урона 1 врагу в вашей зоне. Если у вас 7 ОЗ или меньше,
 * наносит 1 дополнительный урон и даёт 3 энергии.
...
describe("StrikeOfFortunePlus - Наносит 3 урона 1 врагу. Если у вас 7 ОЗ или меньше, наносит 4 урона и даёт 3 энергии.", () => {
  let game: TestGame;
  ...
  it("наносит 3 урона при HP игрока выше 7", async () => {
    ...
    const cardId = await ensureCardInHand(player, admin, ECard.StrikeOfFortunePlus);
  ...
  it("наносит 4 урона и даёт 3 энергии при HP игрока 7 или меньше", async () => {
    game = await createTestGame();
    const [player] = game.players;
    const { admin } = game;
    ...
    const enemy = player.enemies[0];
    await admin.updateEnemy(enemy.id, { hp: 10, shield: 0 });
    await admin.updatePlayer(player.playerId, { hp: 7, energy: 0 });
    ...
    expect(response.player.energy).toBe(3);
  ...
  it("уменьшает AP игрока на 1 и перемещает карту в сброс", async () => {
    ...
    expect(response.steps).toContainEqual(
      expect.objectContaining({
        ...
        to: "discard",
        card: expect.objectContaining({ name: ECard.StrikeOfFortunePlus }),
      }),
    );
  });
  ...
  it("второй игрок тоже получает событие game.useCard", async () => {
    ...
    const cardId = await ensureCardInHand(player1, admin, ECard.StrikeOfFortunePlus);
    ...
    expect(response1.card).toBe(ECard.StrikeOfFortunePlus);
    expect(response2.card).toBe(ECard.StrikeOfFortunePlus);
